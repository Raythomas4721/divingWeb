...
export class UsedproductshowComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.imagePreviews.length === 0) {
      this.showImageError = true;  // 讓 `small` 顯示錯誤訊息
      return;
    }
  ...
  onImageSelected(event: any): void {
    this.showImageError = false; // ✅ 清除錯誤訊息
  ...
  autoFillFormWithoutImage(): void {
    this.UPForm.patchValue({
      categoryId: this.usedCategory.length > 0 ? this.usedCategory[0].categoryId : '',
      productConditionId: this.usedCondition.length > 0 ? this.usedCondition[0].productConditionId : ''
    });

    console.log("✅ 一鍵輸入 (無圖片、名稱、描述、價格) 已填充");
  }
