...
export class SiteRentalComponent implements OnInit {
  ...
  loadSiteDetails(): void {
    this.tsiterentalService.getsiterental(this.selectedRegion, this.currentPage, this.pageSize).subscribe({
      next: (res: any) => {
        this.siteDetails = res.items;
  ...
  searchSites(keyword: string): void {
    ...
    this.tsiterentalService.searchSites(keyword, this.currentPage, this.pageSize).subscribe({
      next: (res: any) => {
        this.siteDetails = res.items; // 匹配 PagedResult.Items
        this.totalItems = res.totalItems;
        this.totalPages = res.totalPages;
        this.currentPage = res.page;
